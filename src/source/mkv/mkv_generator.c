...
// Fragment metadata (implemented as MKV tags)
int Mkv_initializeTagsHdr(const uint32_t numTags, const MkvTag_t tagsList[], MkvTagsBuffer_t* const out) {
    if (tagsList == NULL) {
    ...
    for (uint32_t i = 0; i < numTags; i++) {
        const size_t keyLen = safe_strlen(tagsList[i].key, MAX_TAG_NAME_LEN);
        const size_t valueLen = safe_strlen(tagsList[i].value, MAX_TAG_VALUE_LEN);
